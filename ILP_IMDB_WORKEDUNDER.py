...
# class MyFuncMovie defines the predicate movie as a recursive predicate
################################################
class MyFuncMovie(PredFunc):
    def __init__(self,name='',trainable=True,predColl=None):
        
        super().__init__(name,trainable)
        self.predColl = predColl
    def pred_func(self,xi,xcs,t):
         
        ind2 = self.predColl.preds_by_name['movie'].inp_list.index('movie(A,C)')
        ind3 = self.predColl.preds_by_name['movie'].inp_list.index('workedUnder(C,B)')
        ind4 = self.predColl.preds_by_name['movie'].inp_list.index('workedUnder(B,C)')
        res = xi[:,ind2] * OR( xi[:,ind3],xi[:,ind4])
...
predColl.add_pred(name='director',arguments=['P'] )
predColl.add_pred(name='actor',arguments=['P'] ) 
predColl.add_pred(name='genre',arguments=['P', 'G'] , variables=[] )
predColl.add_pred(name='isFemale',arguments=['P'] )
#we use a recursive formula for predicate movie
predColl.add_pred(name='movie',arguments=['M', 'P'] , variables=['P','G'] ,pFunc = MyFuncMovie(predColl=predColl) , use_neg=True, exc_conds=[('*','rep1') ],  Fam='or' )
# this is the target predicate
predColl.add_pred(name='workedUnder',arguments=['P','P'] , variables=['P','M'] ,pFunc = DNF('workedUnder',terms=1,init=[1,.1,0,.5],sig=1)  , use_neg=True, exc_conds=[('*','rep1') ] , exc_terms=[],  Fam='or')
...
def bgs(it,is_train):
    ...
    if is_train:
        ...
        inds = [ i for i in range(5)  if i != TEST_SET_INDEX]
        index = np.random.randint(4)
        return [ all_bgs[ inds[ index] ] ]
    else:
        return [ all_bgs[TEST_SET_INDEX] ]
